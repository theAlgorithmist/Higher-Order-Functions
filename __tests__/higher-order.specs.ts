...
describe('Higher-Order Function Tests', () => {
  ...
  it('dynamicFilter returns empty array for incorrect and empty inputs', function() {
    expect(dynamicFilter<number>(null, []).length).toBe(0);
    expect(dynamicFilter<number>('return x > 0', null).length).toBe(0);
  ...
  it('reverseFilter returns empty array for incorrect and empty inputs', function() {
    expect(reverseFilter<string>(null, []).length).toBe(0);

    const f: (x: string) => boolean = (x: string): boolean => {return x.length > 0};
    expect(reverseFilter<string>(f, null).length).toBe(0);
  ...
  it('dynamicXform returns empty array for incorrect and empty inputs', function() {
    expect(dynamicXform<number>(null, []).length).toBe(0);
    expect(dynamicXform<number>('return x + 1', null).length).toBe(0);
  ...
  it('extendedReducer returns empty array for incorrect and empty inputs', function() {
    expect(extendedReducer<number>(null, 0, [])).toBeNull();
    expect(extendedReducer<number>((x: number) => x+1, 0, null)).toBeNull();
  ...
  it('hasAny returns empty array for incorrect and empty inputs', function() {
    expect(hasAny<number>(null, [])).toBe(false);
    expect(hasAny<number>('x === 0', null)).toBe(false);
  ...
  it('hasNone returns empty array for incorrect and empty inputs', function() {
    expect(hasNone<number>(null, [])).toBe(false);
    expect(hasNone<number>('x === 0', null)).toBe(false);
  ...
  it('partition returns null for incorrect and empty inputs', function() {
    expect(partition<object>(null, [])).toBeNull();
    expect(partition<object>((x: object) => x['prop'] > 0, null)).toBeNull();
    expect(partition<object>((x: object) => x['prop'] < 0, [])).toBeNull();
  });
  ...
  it('partition tests', function() {
    ...
    const left: Array<number>  = result.left;
    const right: Array<number> = result.right;
